...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    static final String TAG = "PullToRefresh_lgh";

    static final float FRICTION = 2.0f;

    public static final int SMOOTH_SCROLL_DURATION_MS = 200;
    public static final int SMOOTH_SCROLL_LONG_DURATION_MS = 325;
    public static final int ACTION_TOP_SLIDING = 0xff0000;

    static final Mode DEFAULT_MODE = Mode.PULL_DOWN_TO_REFRESH;

    static final String STATE_STATE = "kala_state";
    static final String STATE_MODE = "kala_mode";
    static final String STATE_CURRENT_MODE = "kala_current_mode";
    static final String STATE_DISABLE_SCROLLING_REFRESHING = "kala_disable_scrolling";
    static final String STATE_SHOW_REFRESHING_VIEW = "kala_show_refreshing_view";
    static final String STATE_SUPER = "kala_super";
    ...
    private int mSubHeaderTextType;
    ...
    public void setFirstRefrshTime(long time) {
        mLastTime = time;
    }
    ...
    public final boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        case MotionEvent.ACTION_MOVE: {
            ...
            // Log.d("PullToRefresh", "current scroll y :" + getScrollY());
            // Log.d("PullToRefresh", "header height :" + mHeaderHeight);
        ...
        case MotionEvent.ACTION_DOWN: {
            ...
            if (isReadyForPull()) {
                ...
                // added by zhangjilei
                switch (mSubHeaderTextType) {
                case 0x0:
                default:
                    setLastUpdatedLabel(formatTimeStamp(mLastTime, System.currentTimeMillis()));
                    break;

                case 0x1:
                    setLastUpdatedLabel(formatTimeStamp1(mLastTime));
                    break;
                }
            }
...
//        LogUtil.d(TAG, "dispath touch event...");
//        if(ev.getAction() == MotionEvent.ACTION_MOVE && isRefreshing() && getScrollY() == 0 &&  mLastTouchY > ev.getY()) {
////            ev.setAction(ACTION_TOP_SLIDING);
...
//    }
    ...
    private boolean isReadyPushUp(MotionEvent event) {
        boolean result = false;
//        if (isRefreshing()) {
//            Log.d(TAG, "last touch y " + mLastMotionY + " curr yY = " + event.getY());
//            if (getScrollY() < 0) {
//                result = true;
//            } else {// 宸茬粡鍦ㄩ《閮?//                if (isReadyForPullDown()) {
//                    if (mLastTouchY > event.getY()) {// 鍚戜笂鎺ㄥ姩
//                        result = false;
//                    } else {
//                        result = isReadyForPullDown();
//                    }
//                } else {
//                    result = false;
//                }
...
//        Log.d(TAG, "touch result = " + result);
    ...
    private void pushup(MotionEvent event) {
        int newScrollY = Math.round(Math.min(mLastMotionY - mInitialMotionY, mHeaderHeight));
        Log.d(TAG, "pushup  scroll to =============>>> " + newScrollY + "current scroll y " + getScrollY());
        if (-newScrollY > 0) {
            newScrollY = 0;
        }
        setHeaderScroll(-newScrollY);
        if (newScrollY != 0) {
            float scale = Math.abs(newScrollY) / (float) mHeaderHeight;
            ...
            if (mIsBeingDragged) {
                ...
                if (mState == State.RELEASE_TO_REFRESH) {
                    ...
                    if (null != mOnRefreshListener) {
                        ...
                        // setLastUpdatedLabel(formatTimeStamp(mLastTime,
    ...
    void onRefreshing(final boolean doScroll) {
        ...
        if (doScroll) {
            if (mShowViewWhileRefreshing) {
                int scrollTo = mCurrentMode == Mode.PULL_DOWN_TO_REFRESH ? -mHeaderHeight : mFooterHeight;
                Log.d(TAG, "smooth Scroll To " + scrollTo);
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (a.hasValue(R.styleable.PullToRefresh_kalaMode)) {
            mMode = Mode.mapIntToValue(a.getInteger(R.styleable.PullToRefresh_kalaMode, 0));
        }

        mLoadingAnimationStyle = AnimationStyle.mapIntToValue(a.getInteger(
                R.styleable.PullToRefresh_kalaAnimationStyle, 0));
        ...
        // Styleables from XML
        if (a.hasValue(R.styleable.PullToRefresh_kalaHeaderBackground)) {
            Drawable background = a.getDrawable(R.styleable.PullToRefresh_kalaHeaderBackground);
            if (null != background) {
                setBackgroundDrawable(background);
            }
        }
        if (a.hasValue(R.styleable.PullToRefresh_kalaAdapterViewBackground)) {
            Drawable background = a.getDrawable(R.styleable.PullToRefresh_kalaAdapterViewBackground);
            if (null != background) {
                mRefreshableView.setBackgroundDrawable(background);
            }
        }
        if (a.hasValue(R.styleable.PullToRefresh_kalaOverScroll)) {
            mOverScrollEnabled = a.getBoolean(R.styleable.PullToRefresh_kalaOverScroll, true);
        }

        if (a.hasValue(R.styleable.PullToRefresh_kalaSubHeaderTextType)) {
            mSubHeaderTextType = a.getInteger(R.styleable.PullToRefresh_kalaSubHeaderTextType, 0);
        }
    ...
    private void pullEvent() {
        ...
        Log.d(TAG, " new Scrolly = " + newScrollY);
        setHeaderScroll(newScrollY);

        if (newScrollY != 0) {
            float scale = Math.abs(newScrollY) / (float) itemHeight;
            Log.d(TAG, " newScrollY != 0  mHanderlayout pully  scale = " + scale);
            switch (mCurrentMode) {
            case PULL_UP_TO_REFRESH:
...
//        Log.d(TAG, "refreshLoadingViewsHeight");
        if (mMode.canPullDown()) {
...
//        Log.d(TAG, "smoothScrollTo y="+y);
    ...
    public static interface OnPullEventListener<V extends View> {
         ...
         * @param direction - One of {@link Mode#PULL_UP_TO_REFRESH} or
         *        {@link Mode#PULL_UP_TO_REFRESH} depending on which direction
    ...
    final class SmoothScrollRunnable implements Runnable {
        ...
        public void run() {
            ...
            if (mContinueRunning && mScrollToY != mCurrentY) {
                /*
                 * if (VERSION.SDK_INT >= VERSION_CODES.ECLAIR_MR1JELLY_BEAN) {
                 * //SDK16.postOnAnimation(PullToRefreshBase.this, this); } else
    ...
    private String formatTimeStamp(long lastTime, long curTime) {
        ...
        String timeStamp = "";
        int diffTime = (int) ((curTime - lastTime) / 1000);

        if (diffTime < SECONDS_PER_MINUTE) {
            if (diffTime <= 0) {
                diffTime = 1;
            }
    ...
    private String formatTimeStamp1(long lastTime) {
        /*
         * final int SECONDS_PER_MINUTE = 60; final int SECONDS_PER_HOUR = 3600;
         * final int SECONDS_PER_DAY = 86400; final int DAYS_PER_WEEK = 7; final
         * int DAYS_PER_YEAR = 365; int day = 0; int hours = 0; int minutes = 0;
         */

        String timeStamp = "";
        /*
         * int diffTime = (int)((curTime - lastTime) / 1000); if(diffTime <
         * SECONDS_PER_MINUTE){ if(diffTime <= 0){ diffTime = 1; } timeStamp =
         * diffTime+"绉掑墠"; }else if(diffTime >= SECONDS_PER_MINUTE && diffTime <
         * SECONDS_PER_HOUR){ minutes = diffTime / SECONDS_PER_MINUTE; timeStamp
         * = minutes + "鍒嗛挓鍓?; }else if(diffTime >= SECONDS_PER_HOUR && diffTime
         * < SECONDS_PER_DAY){ hours = diffTime / SECONDS_PER_HOUR; timeStamp =
         * hours + "灏忔椂鍓?; }else if(diffTime >= SECONDS_PER_DAY && diffTime <
         * DAYS_PER_WEEK*SECONDS_PER_DAY){ day = diffTime / SECONDS_PER_HOUR;
         * timeStamp = day + "澶╁墠"; }else{
         */
        SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd HH:mm");
        Date lastDate = new Date(lastTime);
        timeStamp = formatter.format(lastDate) + " ";
