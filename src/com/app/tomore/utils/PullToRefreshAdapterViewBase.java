...
public abstract class PullToRefreshAdapterViewBase<T extends AbsListView> extends PullToRefreshBase<T> implements
	...
	protected void handleStyledAttributes(TypedArray a) {
		...
		mShowIndicator = a.getBoolean(R.styleable.PullToRefresh_kalaShowIndicator, !isPullToRefreshOverScrollEnabled());
	}
		...
		if (null == adapter || adapter.isEmpty()) {
			...
			if (lastVisiblePosition >= lastItemPosition - 1) {
				...
				if (lastVisibleChild != null) {
					int bootem = lastVisibleChild.getBottom();
					int bootem2 = mRefreshableView.getBottom();
					if(bootem >= bootem2){
						return true;
					}else{
						return false;
	...
	private void updateIndicatorViewsVisibility() {
		if (null != mIndicatorIvTop) {
			if (!isRefreshing() && isReadyForPullDown()) {
				if (!mIndicatorIvTop.isVisible() && mShowIndicator) {
